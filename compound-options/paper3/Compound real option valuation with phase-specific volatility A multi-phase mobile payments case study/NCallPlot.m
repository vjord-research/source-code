function [P,Vbar]=NCallPlot(VGrid,r,vol_i,t00,t_i,K_i,p_i,d_i,init,MaxStep,StepSize,GraphsIndic)
rand('seed',0);
n=length(t_i);
N=length(VGrid);
P=zeros(n,N);
...
for s=n:-1:1
    for i=1:1:N      
         [P(s,i),Vbar]=NCall_vol(VGrid(i),r,vol_i([s:end],1),t00,t_i([s:end],1),K_i([s:end],1),p_i([s:end],1),d_i([s:end],1),init,MaxStep,StepSize);               
    end
end;
