function [x0,x0ind] = NCDFinv(x,y,y0,bound, method)
...
% infx - theoretical inf point of the domain of the CDF function
% supx - theoretical sup point of the domain of the CDF function
% method - the algorithm used:
% 0 - standard numerical inverse through interpolation of the grid
...
% Output:
% x0 - inverse value
% x0ind - point of grid at for which the inverse occurs

x0=zeros(1,length(y0(1,:)));
x0ind=zeros(1,length(y0(1,:)));
if method==0
    xx=cumsum(hist(y,length(y)));
    for i=1:1:length(y0)
        x0(1,i)=interp1(y, xx, y0(1,i));
    end
    temp1 = isnan(x0);k1 = find(temp1==1);
    if ~isempty(k1)
        k11=k1(1,end);
...
if method==1
    for i=1:1:length(y0)
        [~, ind] = unique(y);
        x0(1,i)=interp1(y(1,ind), x(1,ind), y0(1,i));
        x0ind(1,i)=i;
    end
    mid=length(x0(1,:))/2;
