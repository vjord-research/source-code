...
methodFVal=1;%the method used for the function value calculation on a grid
trimNaNind=0;%to trim or not any NaN values that could appear
...
% marginals
margdistr='LogNormal';
mPar1=[0.3 0.5]; % mean in case of normal, lognormal, etc.
...
% copula
distr='Gaussian';
copPar1=0.5; %correlation or other
copPar2=[];
...
[Fx1inv,Fx1invInd]=NCDFinv(x1F,Fx1,x1(1,:),bound,methodInv);
[Fx2inv,Fx2invInd]=NCDFinv(x2F,Fx2,x2(1,:),bound,methodInv);
...
LZiTemp=zeros(N2,N1,iter);
SpearmanRhoi=zeros(iter,1);
corrproxi=zeros(N2,N1,iter);
tempx1=zeros(iter,N1);tempx2=zeros(iter,N2);
tempx11=zeros(iter,N1);tempx22=zeros(iter,N2);
ExpLZi=zeros(iter,1);
ExpLZ1i=zeros(iter,1);
ExpLZx1i=zeros(iter,1);ExpLZx1ii=zeros(iter,1);
ExpLZx2i=zeros(iter,1);ExpLZx2ii=zeros(iter,1);
...
if trimNaNind==1
    ...
    FvTemp=Fval(X1F,X2F,F,S1,S2,s1Ind,s2Ind,methodFVal);
    FvTemp1=Fval(X1F,X2F,F,S1,S2,s1Ind,s2Ind,methodFVal);
    Fv=TrimNaN(FvTemp);
    prodTemp=S1.*S2.*Fv;
    ...
    s1Ind = Fx1invInd(1,:);s2Ind = Fx2invInd(1,:);
    [S1,S2] = meshgrid(s1,s2);
    Fv=Fval(X1F,X2F,F,S1,S2,s1Ind,s2Ind,methodFVal);
    prod=S1.*S2.*Fv;
end

indx1F=x1F<=1;x1Fnew=x1F(1,indx1F);
indx2F=x2F<=1;x2Fnew=x2F(1,indx2F);
...
for i=2:1:iter
    [LZx1invi(i-1,:),LZx1inviInd(i-1,:)]=NCDFinv(x1,LZx1i(i-1,:),x1(1,:),bound,methodInv);
    [LZx2invi(i-1,:),LZx2inviInd(i-1,:)]=NCDFinv(x2,LZx2i(i-1,:),x2(1,:),bound,methodInv); 
    [LZx1invii(i-1,:),LZx1inviiInd(i-1,:)]=NCDFinv(x1,LZx1ii(i-1,:),x1(1,:),bound,methodInv);
    [LZx2invii(i-1,:),LZx2inviiInd(i-1,:)]=NCDFinv(x2,LZx2ii(i-1,:),x2(1,:),bound,methodInv);
    if trimNaNind==1
        ...
        FviTempi=Fval(X1,X2,LZi(:,:,i-1),S1i,S2i,s1iInd,s2iInd,methodFVal);
        ...
        s1iInd = LZx1inviInd(i-1,:);s2iInd = LZx2inviInd(i-1,:);
        [S1i,S2i] = meshgrid(s1i,s2i);
        Fvi=Fval(X1,X2,LZi(:,:,i-1),S1i,S2i,s1iInd,s2iInd,methodFVal);
        prodi=S1i.*S2i.*Fvi;
    end
    ...
    ExpLZ1i(i-1,1)=1-trapz(s1i,trapz(s2i,LZi(:,:,i-1)',2));
    ...
    corrproxi(:,:,i)=LZi(:,:,i-1)./(LZx1iT.*LZx2iT);
...
title('Independence check1: Spearman Rho through iterations')

figure(17)
for i=iter
    surf(X1,X2,corrproxi(:,:,i))
    shadowplot x   
    shadowplot y
    hold on
    title('Independence check2: cdf/(cdfX1*cdfX2)=1 for the limit')
end
