function [P_s,M]=Price_se(V,s,Vbar,r,vol,t0,t,alpha,beta,init,MaxStep,StepSize)
...
s1=nn;
TimeM=zeros(s1,2^s1+1);
StrikeM=zeros(s1,2^s1+1);

WeightM=zeros(1,2^s1+1);
PriceM=zeros(1,2^s1+1);
