function [P,Vbar]=NExp(V,r,vol,t00,t_i,alpha_i,beta_i,p_i,d_i,init,MaxStep,StepSize)
...
if n>1
    for s=n:-1:2 % loop for finding Vbar_s
        ...
        while (exitflag1~=1 && it<lnt);        
            ...
            [x1,fval1,exitflag1,output1]=fzero(@Price_sse,initial,options,s,Vbar,r,vol,t0,t,alpha,beta,init,MaxStep,StepSize);%finding Vbar_s-1         
            it=it+1;
        end
         if exitflag1==1 
            Vbar(s-1,1)=x1*x1;
    ...
    if indic==1
        ...
        [P]=Price_se(V,k,Vbar,r,vol,t00,t,alpha,beta,init,MaxStep,StepSize);
    else
        P='ERROR';
    end
else
    kk=1; % finding the price
    [P]=Price_se(V,kk,Vbar,r,vol,t00,t,alpha,beta,init,MaxStep,StepSize);
end
