function [P,Vbar]=NDigitalCN_vol(V,r,vol_i,t00,t_i,K_i,p_i,d_i,init,MaxStep,StepSize)
...
if n>1
    for s=n:-1:2 % loop for finding Vbar_s
        ...
        while (exitflag1~=1 && it<lnt)         
            ...
            [x1,fval1,exitflag1,output1]=fzero(@Price_ssdigCN_vol,initial,options,s,Vbar,r,vol,t0,t,K);%finding Vbar_s-1        
            it=it+1;     
        end
        if exitflag1==1 
            Vbar(s-1,1)=x1*x1;
        ...
        [c1,h1,a1,b1,N1,w2,w1]=Price_sdigCN_vol(V,s,Vbar,r,vol,t0,t,K);
        C(s,1)=c1;h(s,:)=h1;a(s,:)=a1;b(s,:)=b1;N(s,:)=N1;
    end
    if ind==1
        k=1; % finding the price
        [c2,h2,a2,b2,N2,w4,w3]=Price_sdigCN_vol(V,k,Vbar,r,vol,t00,t,K);
    ...
    [c2,h2,a2,b2,N2,w4,w3]=Price_sdigCN_vol(V,kk,Vbar,r,vol,t00,t,K);
