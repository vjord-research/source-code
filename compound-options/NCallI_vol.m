function [P,Vbar]=NCallI_vol(V,r,vol,t00,t_i,gamma_i,K_i,p_i,d_i,init,MaxStep,StepSize)
...
% 4)t00 - today 
% 6)gamma_i - (n,1) vector of scale factors
% 7)K - (n,1) vector of strikes
% 8)d_i - (n,1) vector of dividends (date 1 is zero by default, so no matter
% what is put there
% 9)p_i - (n,1) vector of technical probabilities risk
% 10)init - number that adjusts initial values in finding the zeros of
...
dimm=length(p_i(:,1));
t=t_i;
K=zeros(dimm,1);gamma=zeros(dimm,1);d=zeros(dimm,1);
V=prod(p_i(:,1))*V;

for i=1:1:dimm %computing the modified strikes
    K(dimm-i+1,1)=K_i(dimm-i+1,1)*prod(p_i([1:dimm-i+1],1));
    gamma(dimm-i+1,1)=gamma_i(dimm-i+1,1)*p_i(dimm-i+1,1);
end
d=d_i;
...
if n>1
    for s=n:-1:2 % loop for finding Vbar_s
        ...
        while (exitflag1~=1 && it<lnt)        
            ...
            [x1,fval1,exitflag1,output1]=fzero(@Price_ssI_vol,initial,options,s,Vbar,r,vol,t0,t,gamma,K,d,init,MaxStep,StepSize);%finding Vbar_s-1          
            it=it+1;
        end
        if exitflag1==1 
            Vbar(s-1,1)=x1*x1;
    ...
    if indic==1
        ...
        [P]=Price_sI_vol(V,k,Vbar,r,vol,t00,t,gamma,d,init,MaxStep,StepSize);
    else
        P='ERROR';
    end
else
    kk=1; % finding the price
    [P]=Price_sI_vol(V,kk,Vbar,r,vol,t00,t,gamma,d,init,MaxStep,StepSize);
end