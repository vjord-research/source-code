function [F] = NPDFx(x,distr,margdistr,copPar1,copPar2,margPar1,margPar2,margPar3,margPar4,margPar5)
% Multivariate PDF based on copula and marginals
...
switch p
    case 2
        ...
        x11p = pdf(margdistr,x1,P11,P21,P31,P41,P51);
        x22p = pdf(margdistr,x2,P12,P22,P32,P42,P52);
        [X1,X2]=meshgrid(x11,x22);
        [X1p,X2p]=meshgrid(x11p,x22p);
        FF=copulapdf(distr,[X1(:),X2(:)],copPar1,copPar2);
        F=reshape(FF,N2,N1).*X1p.*X2p;
    case 3
        ...
        x11p = pdf(margdistr,x1,P11,P21,P31,P41,P51);
        x22p = pdf(margdistr,x2,P12,P22,P32,P42,P52);
        x33p = pdf(margdistr,x3,P13,P23,P33,P43,P53);
        [X1,X2,X3]=meshgrid(x11,x22,x33);
        [X1p,X2p,X3p]=meshgrid(x11p,x22p,x33p);
        FF=copulapdf(distr,[X1(:),X2(:),X3(:)],copPar1,copPar2);
        F=reshape(FF,N3,N2,N1).*X1p.*X2p.*X3p;
end
